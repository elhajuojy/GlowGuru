...
if (Validator::string($name) && Validator::string($cateogry) && Validator::string($product_brand) && Validator::string($product_price) && Validator::string($product_quantity) && Validator::string($product_description) && Validator::string($product_id)) {
    ...
    $db->query(
        "UPDATE makeup_products SET product_name = :name, product_category = :category, brand = :brand, product_price = :price, product_quantity = :quantity, product_description = :description WHERE id = :id",
        [
            'name' => $name,
            'category' => $cateogry,
            'brand' => $product_brand,
